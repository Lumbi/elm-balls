...
Elm.Main.make = function (_elm) {
   ...
   var _op = {};
   var tick = $Time.fps(60);
   var viewBall = function (ball) {
      return A2($Graphics$Collage.move,{ctor: "_Tuple2",_0: ball.x,_1: ball.y},A2($Graphics$Collage.filled,ball.color,$Graphics$Collage.circle(ball.radius)));
   };
   var viewBalls = function (balls) {    return $Graphics$Collage.group(A2($List.map,viewBall,balls));};
   var view = F2(function (_p0,state) {
      var _p1 = _p0;
      return A3($Graphics$Collage.collage,$Basics.round(_p1._0),$Basics.round(_p1._1),_U.list([viewBalls(state.balls)]));
   });
   var collisionBetweenBalls = F2(function (ball,otherBall) {
      var radiSquared = Math.pow(otherBall.radius + ball.radius,2);
      var distanceSquared = Math.pow(otherBall.x - ball.x,2) + Math.pow(otherBall.y - ball.y,2);
      return _U.eq(ball.id,otherBall.id) ? $Maybe.Nothing : _U.cmp(distanceSquared,radiSquared) < 0 ? $Maybe.Just({ctor: "_Tuple2"
                                                                                                                  ,_0: ball
                                                                                                                  ,_1: otherBall}) : $Maybe.Nothing;
   });
   var collisionForBall = F2(function (ball,balls) {
      return A3($List.foldl,
      F2(function (currentBall,maybeCollision) {
         var _p2 = maybeCollision;
         if (_p2.ctor === "Just") {
               return A2($Debug.log,"Collision!",$Maybe.Just(_p2._0));
            } else {
               return A2(collisionBetweenBalls,ball,currentBall);
            }
      }),
      $Maybe.Nothing,
      balls);
   });
   var physicsBall = F2(function (dt,ball) {
      var newVy = ball.vy + dt * ball.ax;
      var newVx = ball.vx + dt * ball.ax;
      return _U.update(ball,{x: ball.x + dt * newVx,y: ball.y + dt * newVy,vx: newVx,vy: newVy});
   });
   var ball214 = {id: 213,radius: 10,x: 35,y: -200,vx: -0.1,vy: -0.1,ax: 0,ay: 0,color: $Color.blue,mass: 1};
   var ball213 = {id: 212,radius: 10,x: -130,y: -165,vx: 0.3,vy: 0.1,ax: 0,ay: 0,color: $Color.yellow,mass: 1};
   var ball212 = {id: 211,radius: 10,x: 100,y: 110,vx: -0.1,vy: 0,ax: 0,ay: 0,color: $Color.red,mass: 1};
   var ball211 = {id: 210,radius: 10,x: -100,y: 50,vx: 0.1,vy: 0.1,ax: 0,ay: 0,color: $Color.green,mass: 1};
   var ball24 = {id: 23,radius: 10,x: 135,y: -30,vx: -0.1,vy: -0.1,ax: 0,ay: 0,color: $Color.blue,mass: 1};
   var ball23 = {id: 22,radius: 10,x: -30,y: 165,vx: 0.3,vy: 0.1,ax: 0,ay: 0,color: $Color.yellow,mass: 1};
   var ball22 = {id: 21,radius: 10,x: 40,y: 110,vx: -0.1,vy: 0,ax: 0,ay: 0,color: $Color.red,mass: 1};
   var ball21 = {id: 20,radius: 10,x: 0,y: -100,vx: 0.1,vy: 0.1,ax: 0,ay: 0,color: $Color.green,mass: 1};
   var ball14 = {id: 13,radius: 10,x: 35,y: -130,vx: -0.1,vy: -0.1,ax: 0,ay: 0,color: $Color.blue,mass: 1};
   var ball13 = {id: 12,radius: 10,x: -30,y: -165,vx: 0.3,vy: 0.1,ax: 0,ay: 0,color: $Color.yellow,mass: 1};
   var ball12 = {id: 11,radius: 10,x: 100,y: 10,vx: -0.1,vy: 0,ax: 0,ay: 0,color: $Color.red,mass: 1};
   var ball11 = {id: 10,radius: 10,x: -100,y: 0,vx: 0.1,vy: 0.1,ax: 0,ay: 0,color: $Color.green,mass: 1};
   var ball4 = {id: 3,radius: 10,x: 35,y: -30,vx: -0.1,vy: -0.1,ax: 0,ay: 0,color: $Color.blue,mass: 1};
   var ball3 = {id: 2,radius: 10,x: -30,y: 65,vx: 0.3,vy: 0.1,ax: 0,ay: 0,color: $Color.yellow,mass: 1};
   var ball2 = {id: 1,radius: 10,x: 40,y: 10,vx: -0.1,vy: 0,ax: 0,ay: 0,color: $Color.red,mass: 1};
   var ball1 = {id: 0,radius: 10,x: 0,y: 0,vx: 0.1,vy: 0.1,ax: 0,ay: 0,color: $Color.green,mass: 1};
   var initialState = {balls: _U.list([ball1,ball2,ball3,ball4,ball11,ball12,ball13,ball14,ball21,ball22,ball23,ball24,ball211,ball212,ball213,ball214])};
   var cr = 0.95;
   var velocityAfterCollision = F2(function (_p4,_p3) {
      var _p5 = _p4;
      var _p10 = _p5._0;
      var _p9 = _p5._1;
      var _p6 = _p3;
      var _p8 = _p6._0;
      var _p7 = _p6._1;
      return (cr * _p7 * (_p8 - _p10) + _p9 * _p10 + _p7 * _p8) / (_p9 + _p7);
   });
   var ballAfterCollision = function (col) {
      var ballB = $Basics.snd(col);
      var ballA = $Basics.fst(col);
      var distance = $Basics.sqrt(Math.pow(ballB.x - ballA.x,2) + Math.pow(ballB.y - ballA.y,2));
      var radi = ballA.radius + ballB.radius;
      var penetration = radi - distance;
      var unitVectorFromAtoB = {x: (ballB.x - ballA.x) / distance,y: (ballB.y - ballA.y) / distance};
      var penetrationVector = {x: penetration * unitVectorFromAtoB.x,y: penetration * unitVectorFromAtoB.y};
      var minimumTranslationVector = {x: 0 - penetrationVector.x / 2,y: 0 - penetrationVector.y / 2};
      return _U.update(ballA,
      {x: ballA.x + minimumTranslationVector.x
      ,y: ballA.y + minimumTranslationVector.y
      ,vx: A2(velocityAfterCollision,{ctor: "_Tuple2",_0: ballA.vx,_1: ballA.mass},{ctor: "_Tuple2",_0: ballB.vx,_1: ballB.mass})
      ,vy: A2(velocityAfterCollision,{ctor: "_Tuple2",_0: ballA.vy,_1: ballA.mass},{ctor: "_Tuple2",_0: ballB.vy,_1: ballB.mass})});
   };
   var collideBallsWithBalls = function (balls) {
      return A2($List.map,
      function (ball) {
         var _p11 = A2(collisionForBall,ball,balls);
         if (_p11.ctor === "Just") {
               return ballAfterCollision(_p11._0);
            } else {
               return ball;
            }
      },
   ...
   var bottom = function (ball) {    return ball.y - ball.radius;};
   var top = function (ball) {    return ball.y + ball.radius;};
   var right = function (ball) {    return ball.x + ball.radius;};
   var left = function (ball) {    return ball.x - ball.radius;};
   var Ball = function (a) {
      return function (b) {
         return function (c) {
            return function (d) {
                  ...
                  return function (f) {
                     return function (g) {
                        return function (h) {
                           return function (i) {
                              return function (j) {
                                 return {id: a,radius: b,x: c,y: d,vx: e,vy: f,ax: g,ay: h,color: i,mass: j};
                              };
                           };
                        };
                     };
   ...
   var Model = function (a) {    return {balls: a};};
   var fieldSize = {w: 400,h: 400};
   var fieldBounds = {left: (0 - fieldSize.w) / 2,right: fieldSize.w / 2,top: fieldSize.h / 2,bottom: (0 - fieldSize.h) / 2};
   var collideBallsWithWalls = function (balls) {
      return A2($List.map,
      function (ball) {
         return _U.cmp(left(ball),fieldBounds.left) < 0 ? _U.update(ball,
         {x: fieldBounds.left + ball.radius,vx: 0 - ball.vx,ax: 0 - ball.ax}) : _U.cmp(top(ball),fieldBounds.top) > 0 ? _U.update(ball,
         {y: fieldBounds.top - ball.radius,vy: 0 - ball.vy,ay: 0 - ball.ay}) : _U.cmp(right(ball),fieldBounds.right) > 0 ? _U.update(ball,
         {x: fieldBounds.right - ball.radius,vx: 0 - ball.vx,ax: 0 - ball.ax}) : _U.cmp(bottom(ball),fieldBounds.bottom) < 0 ? _U.update(ball,
         {y: fieldBounds.bottom + ball.radius,vy: 0 - ball.vy,ax: 0 - ball.ax}) : ball;
      },
      balls);
   };
   var physics = F2(function (dt,state) {
      return _U.update(state,{balls: A2($List.map,physicsBall(dt),collideBallsWithBalls(collideBallsWithWalls(state.balls)))});
   });
   var update = F2(function (dt,state) {    return A2(physics,dt,state);});
   var updatedModel = A3($Signal.foldp,update,initialState,tick);
   var main = A2($Signal.map,view({ctor: "_Tuple2",_0: fieldSize.w,_1: fieldSize.h}),updatedModel);
   return _elm.Main.values = {_op: _op
                             ...
                             ,collideBallsWithWalls: collideBallsWithWalls
                             ,collideBallsWithBalls: collideBallsWithBalls
                             ,collisionForBall: collisionForBall
                             ,collisionBetweenBalls: collisionBetweenBalls
                             ,ballAfterCollision: ballAfterCollision
                             ,velocityAfterCollision: velocityAfterCollision
                             ,view: view
                             ,viewBalls: viewBalls
                             ,viewBall: viewBall
                             ,tick: tick
